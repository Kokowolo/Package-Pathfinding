...
## [0.0.24] - 2026-02-16
### Added
* `IDisposable` to `NodePath`
### Changed
* function ordering and naming within `IPathfinder`
* `guids` for some scripts
### Removed
* `Pathfinder.IsPathOutsideMovementRange` since this is effectively a `maxDistance` check
* `AStarPathfinding.TryReduceOutsideMovementRangePath` since this is done during the `maxDistance` check

## [0.0.23] - 2026-02-07
### Added
* pooling to `DebugNodeObject`
### Fixed
* use of `DebugNodeObject` using `TextMeshPro` rather than `TextMeshProUGUI`

## [0.0.22] - 2026-02-02
### Added
* `SetTitleTextColor` to `DebugNodeObject`
### Changed
* `NodeDebugObject` to `DebugNodeObject`
* `DebugNodeObject`'s colors to be static and public
* `AStarPathfinding`'s `searchPath` member to be exposed as a property

## [0.0.21] - 2026-01-29
### Fixed
* bug where `NodeDebugObject` was updating to the wrong `INode` property

## [0.0.20] - 2026-01-25
### Added
* `INodeComposition` to have an interface for handling a `Node` object
* `IPathfinder` splitting up functionality of `IPathfinding` to a manager interface and an instance interface
### Changed
* AStarPathfinding to use `INode` without a `INode.Node` member
* `INode` to have all properties that `Node` had and `Node` to serve as an object containing `INode` properties
* all classes starting with `INode`[...] to now lead with `Node`[...]
### Removed
* all confusing neighbor functions within `Node`

## [0.0.19] - 2026-01-07
### Changed
* all of `AStarPathfinding` to use `INode` instead of `Node`
* `NodePath` to `INodePath`
* `NodePriorityQueue` to `INodePriorityQueue`
### Removed
* `Node<T>` and `INode<T>` since `INode` is now the core of pathfinding 

## [0.0.18] - 2025-06-12
### Added
* property `NumberOfNeighbors` to `Node`
* function `RemoveNeighbor` to `Node`
